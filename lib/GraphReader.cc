...
Token Lexer::next_token() {
    ...
    if (ch_ == '[')   { ch_ = in_.get(); return TOK_LEFT_SQUARE; }
    if (ch_ == ']')   { ch_ = in_.get(); return TOK_RIGHT_SQUARE; }
                                       ...
                                       bool partitionA)
{
    ...
    while (curtok_ != TOK_SEMICOLON) {
        if (curtok_ == TOK_LEFT_SQUARE) {
            // eat '['
            match(TOK_LEFT_SQUARE);
            int flag = 0;
            while(curtok_ != TOK_RIGHT_SQUARE){
                std::string b = lexer_->get_lexeme();
                match(TOK_STRING);
                if(flag == 0){
                    pref_list.emplace_back_ties(partners[b],false);                
                    flag = 1;
                }
                else{
                    pref_list.emplace_back_ties(partners[b],true);                              
                }
                if (curtok_ == TOK_COMMA) {
                    match(TOK_COMMA);
                    continue;
